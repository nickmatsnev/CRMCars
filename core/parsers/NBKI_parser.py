...
def stop_factors(individual_json, source_json):
    ...
    if scorista_res.loc['totalOverDue'].value > 300000:
        errors.append({'decription': 'Суммарная просроченная задолженность по всем активным счетам более 300000 рублей'})
        
    if scorista_res.loc['delay'].value > 100000:
        errors.append({'decription': 'Максимальная текущая просрочка по активным кредитам, по справочнику PMTPAT, более 100000 рублей'})
        
    if scorista_res.loc['maxdeleay'].value > 300000:
        errors.append({'decription': 'Максимальная историческая просрочка по активным кредитам, по справочнику PMTPAT более 300000 рублей'})
    ...
    if scorista_res.loc['countdue30_60inopenedaccs'].value > 6:
        errors.append({'decription': 'Количество просрочек 30 - 59 дней за последние 12 месяцев по открытым счетам более 6'})
        
    if scorista_res.loc['countdue30_60inclosedaccs'].value > 6:
        errors.append({'decription': 'Количество просрочек 30 - 59 дней за последние 12 месяцев по закрытым счетам более 6'})
        
    if scorista_res.loc['countdue60_90inopenedaccs'].value > 6:
        errors.append({'decription': 'Количество просрочек 60 - 89 дней за последние 12 месяцев по открытым счетам более 6'})
        
    if scorista_res.loc['countdue60_90inclosedaccs'].value > 6:
        errors.append({'decription': 'Количество просрочек 60 - 89 дней за последние 12 месяцев по закрытым счетам более 6'})
...
def get_available_params():
    return [{'name': 'Passport', 'description': 'Серия и номер паспорта', 'type': 'int'},
                ...
                {'name': 'totalOverDue', 'description': 'Суммарная просроченная задолженность по всем активным счетам', 'type': 'int'},
                {'name': 'delay', 'description': 'Максимальная текущая просрочка по активным кредитам по справочнику PMTPAT', 'type': 'int'},
                {'name': 'maxDelay', 'description': 'Максимальная историческая просрочка по активным кредитам по справочнику PMTPAT', 'type': 'int'},
                {'name': 'closednegative', 'description': 'Наличие негатива в закрытых кредитах', 'type': 'string'},
                {'name': 'countdue30_60inopenedaccs', 'description': 'Количество просрочек 30 - 59 дней за последние 12 месяцев по открытым счетам', 'type': 'int'},
                {'name': 'countdue30_60inclosedaccs', 'description': 'Количество просрочек 30 - 59 дней за последние 12 месяцев по закрытым счетам', 'type': 'int'},
                {'name': 'countdue60_90inopenedaccs', 'description': 'Количество просрочек 60 - 89 дней за последние 12 месяцев по открытым счетам', 'type': 'int'},
                {'name': 'countdue60_90inclosedaccs', 'description': 'Количество просрочек 60 - 89 дней за последние 12 месяцев по закрытым счетам', 'type': 'int'},
                {'name': 'countdue90plusinopenedaccs', 'description': 'Количество просрочек 90+ дней за все время по открытым счетам', 'type': 'int'},
                {'name': 'countdue90plusinclosedaccs', 'description': 'Количество просрочек 90+ дней за все время по закрытым счетам', 'type': 'int'},
